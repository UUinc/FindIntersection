...
void FindIntersection(char * strArr[], int arrLength) {
	...
	for(i=0, j=0; i < len1;i++)
	{
	    ...
	    if(j != c)
            A[j] = A[j] * 10 + charToInt(strArr[0][i]);
        else
            A[j] = charToInt(strArr[0][i]);

        c++;
	}
	//Set len for the new array A
	len1 = j+1;
	...
	for(i=0, j=0; i < len2;i++)
	{
	    ...
	    if(j != c)
            B[j] = B[j] * 10 + charToInt(strArr[1][i]);
        else
            B[j] = charToInt(strArr[1][i]);

        c++;
	}
	//Set len for the new array B
	len2 = j+1;
...
int main(void) {
	...
	char * A[] = {"1, 3, 9, 10, 17, 18", "1, 4, 9, 10"};//Here to put your 2d array
